...
class CheckoutController extends Controller
{
    ...
    public function checkout(){
    	...
    	$cate_product = DB::table('tbl_category_product')->where('category_status','0')->orderby('category_id','desc')->get();
    	$brand_product = DB::table('tbl_brand')->where('brand_status','0')->orderby('brand_id','desc')->get();
    	return view('pages.checkout.show_checkout')->with('category',$cate_product)->with('brand',$brand_product)->with('category_post',$category_post)->with('city',$city);
    }
    ...
    public function order_place(Request $request){
        ...
        $payment_id = DB::table('tbl_order_details')->insert($order_d_data);
        }
        if($data['payment_method']==1){
            ...
            $cate_product = DB::table('tbl_category_product')->where('category_status','0')->orderby('category_id','desc')->get();
            $brand_product = DB::table('tbl_brand')->where('brand_status','0')->orderby('brand_id','desc')->get();
            return view('pages.checkout.cash')->with('category',$cate_product)->with('brand',$brand_product)->with('category_post',$category_post);
        }
        //return Redirect::to('/payment');
    }
    ...
    public function login_customer(Request $request){
    	...
    	if($result){
            ...
            Session::put('message','Mật khẩu hoặc tên đăng nhập sai !!!');
            return Redirect::to('/login-checkout');
        }
    ...
    public function delete_order($orderId){
        $this->AuthLogin();
        $all_order = DB::table('tbl_order')->where('order_id',$orderId)->delete();
        $manage_order = view('admin.manage_order')->with('all_order',$all_order);
